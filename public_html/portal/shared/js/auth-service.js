(function(){
  ...
  var LEGACY_PERMISSIONS = {
    ...
    admin: ['admin.dashboard','admin.reports','admin.help','admin.wiki','admin.analytics','admin.crm','admin.agreements','admin.students'],
    super_admin: ['*']
  };
  ...
  function safePortalNext(value, fallback){
    ...
    try{
      ...
      if(url.origin === location.origin && url.pathname.indexOf('/portal/') === 0){
  ...
  function getSupabase(){
    return window.GEESSupabase || null;
  }

  function isDemoEmail(email){
    return /@gees\.demo$/i.test(String(email || '').trim());
  }
  ...
  async function getPortalSession(){
    try{
      ...
      console.warn('[GEES Auth] Supabase session check failed; falling back to local demo session.', error);
    }
  ...
  async function login(options){
    ...
    if(isDemoEmail(email)){
      ...
      localStorage.setItem(STORAGE.lastAuthMode, 'demo');
      return { mode: 'demo', session: demoSession, next: safePortalNext(options.next, demoSession.dashboard) };
    }

    var client = getSupabase();
    if(!client){
      throw new Error('Supabase client is unavailable. Use demo credentials or check your internet/CDN connection.');
    }

    var authResponse = await client.auth.signInWithPassword({ email: email, password: password });
    if(authResponse.error){
      throw new Error(authResponse.error.message || 'Supabase login failed.');
    }
    if(!authResponse.data || !authResponse.data.user){
      throw new Error('Supabase login did not return a user session.');
    }
    ...
    if(!profile){
      ...
      clearStoredSession();
      throw new Error('Your GEES profile is missing. Please contact an admin.');
    }

    var actualRole = normaliseRole(profile.role);
    if(!expectedRoleAllows(expectedRole, actualRole)){
      await client.auth.signOut();
      clearStoredSession();
    ...
    if(profile.status !== 'active'){
      await client.auth.signOut();
      clearStoredSession();
      throw new Error('Your GEES account is currently ' + profile.status + '. Please wait for admin approval.');
    }
    ...
    localStorage.setItem(STORAGE.lastAuthMode, 'supabase');
  ...
  async function signup(options){
    ...
    if(['student','agent','staff'].indexOf(role) === -1){
      throw new Error('Only student, agent, and staff signup are open from the website. Admin users must be created manually.');
    }
    var client = getSupabase();
    if(!client){
      throw new Error('Supabase client is unavailable. Signup is temporarily in demo/setup mode.');
    }
    var email = String(options.email || '').trim().toLowerCase();
    var password = String(options.password || '');
    ...
    var metadata = Object.assign({}, options.metadata || {}, {
      ...
      phone: options.phone || '',
      team_id: role === 'staff' ? (options.teamId || 'staff') : role
    });
    ...
    var response = await client.auth.signUp({
      ...
      options: {
        data: metadata,
        emailRedirectTo: location.origin + '/portal/auth/' + role + '-login.html'
      }
    });
    if(response.error){
      throw new Error(response.error.message || 'Supabase signup failed.');
    }
    return {
      mode: 'supabase',
      user: response.data && response.data.user,
      session: response.data && response.data.session,
      status: role === 'student' ? 'active_or_email_confirmation' : 'pending_admin_approval',
      message: role === 'student'
        ? 'Signup submitted. Check your email if confirmation is enabled, then sign in.'
        : 'Signup submitted. Your account is pending GEES admin approval.'
    };
  }


  async function listPendingUserApprovals(){
    var client = getSupabase();
    if(!client) throw new Error('Supabase client is unavailable.');
    var response = await client.rpc('get_pending_gees_user_approvals');
    if(response.error) throw new Error(response.error.message || 'Unable to load pending approvals.');
    return response.data || [];
  }

  async function approveUser(userId, note){
    var client = getSupabase();
    if(!client) throw new Error('Supabase client is unavailable.');
    var response = await client.rpc('approve_gees_user', {
      p_user_id: userId,
      p_note: note || null
    });
    if(response.error) throw new Error(response.error.message || 'Unable to approve user.');
    return response.data;
  }

  async function rejectUser(userId, note){
    var client = getSupabase();
    if(!client) throw new Error('Supabase client is unavailable.');
    var response = await client.rpc('reject_gees_user', {
      p_user_id: userId,
      p_note: note || null
    });
    if(response.error) throw new Error(response.error.message || 'Unable to reject user.');
    return response.data;
  }
  ...
  async function logout(){
    ...
    if(window.GEESDemoBackend && typeof window.GEESDemoBackend.logout === 'function'){
      try{ window.GEESDemoBackend.logout(); }catch(error){}
    }
    clearStoredSession();
  }

  window.GEESAuthService = {
    storage: STORAGE,
    dashboards: ROLE_DASHBOARD,
    normaliseRole: normaliseRole,
    dashboardFor: dashboardFor,
    safePortalNext: safePortalNext,
    expectedRoleAllows: expectedRoleAllows,
    getPortalSession: getPortalSession,
    getRealSession: getRealSession,
    login: login,
    signup: signup,
    listPendingUserApprovals: listPendingUserApprovals,
